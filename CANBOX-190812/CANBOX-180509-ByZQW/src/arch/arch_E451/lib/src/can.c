...
CANRegRead(unsigned long ulRegAddress)
{
    ...
    //
	ulReenableInts = 0;//HWREG(NVIC_EN1) & (1 << (ulIntNumber - 48));
                ...
                unsigned long ulSize)
{
    ...
    for(ulIdx = 0; ulIdx < ulSize; )
    {
		...
		// 1 ulValue |= (pucData[ulIdx] << 8);
		// 2 ulIdx = ulIdx + 1;
        CANRegWrite((unsigned long)(pulRegister++), ulValue);
		// 1 CANRegWrite((unsigned long)(pulRegister),ulValue);
		// 2 pulRegister = pulRegister + 1;
    }
}
               ...
               unsigned long ulSize)
{
    ...
    for(ulIdx = 0; ulIdx < ulSize; )
    {
		...
		//(unsigned long)(pulRegister++)let the address CANIFnDA1 to the next address

        //
        // Store the first byte.
        //
        pucData[ulIdx++] = (unsigned char)ulValue;
		// Store the byte first then plus the ulInx
...
//! CAN模块给CAN提供32个独立的消息对象每个CAN都是互相独立的,CAN的消息对象
//! 所代表的的数值越小,其优先级最高
...
//! 若需要使用ID过滤,则需要在前面就设置需要过滤的ID号,并且在ulflags中将
//! MSG_OBJ_USE_ID_FILTER通过与运算添加进去
              ...
              tCANMsgObject *pMsgObject, tMsgObjType eMsgType)
{
    ...
    //
    // Wait for busy bit to clear
    // The fucntion CANRegRead() is used to read the value 
    ...
    //
    usArbReg0 = 0;		//message object's ID(low for enterned ID)
    usArbReg1 = 0;		//message object's ID
    usMsgCtrl = 0;
    usMaskReg0 = 0;		//message object's filter ID
    usMaskReg1 = 0;		//
    ...
    //
    if((pMsgObject->ulFlags&MSG_OBJ_USE_EXT_FILTER)==MSG_OBJ_USE_EXT_FILTER)
    {
    ...
    // 扩展ID MASK的高13位或者是标准IDMASK的11位

	CANRegWrite(ulBase + CAN_O_IF1ARB1, usArbReg0);
	CANRegWrite(ulBase + CAN_O_IF1ARB2, usArbReg1);
    // 扩展ID的高13位或者是标准ID的11位,direction,message valided or not and 
    // CANIFnARBn中的ID是用来做ID过滤用的

	CANRegWrite(ulBase + CAN_O_IF1MCTL, usMsgCtrl);
	// 接发中断,数据长度,是否使用ID过滤,远程帧使能
              ...
              tCANMsgObject *pMsgObject, tBoolean bClrPendingInt)
{
    ...
    unsigned short usArbReg0, usArbReg1;
    unsigned short usMsgCtrl;
	//this variable instead of the value of corresponding register
    ...
    //
    // Read out the IF(information) Registers.
    //
	//读取ID的MASK
    usMaskReg0 = CANRegRead(ulBase + CAN_O_IF2MSK1);	//低位
    usMaskReg1 = CANRegRead(ulBase + CAN_O_IF2MSK2);	//高位

	//读取ID
    usArbReg0 = CANRegRead(ulBase + CAN_O_IF2ARB1);		//低位
    usArbReg1 = CANRegRead(ulBase + CAN_O_IF2ARB2);		//高位
    ...
    {//while DIR == 0&&TXRQST == 1 indicates that a remote frame received.
    ...
    else
    {
		...
		// 在此处需要>>2,因为标准ID的ulMsgID保存在寄存器CANIf1ARB2的bit[12:2];
		// 而在其读取的时候,读取的是整个寄存器的值,即寄存器CANIf1ARB2的bit[15:0]
		// 所以要先与上0x00001FFF,然后>>2.
    }//这样一来就相当于将ID保存到pMsgObject->ulFlags中去了
    ...
    {//using IDMask
        ...
        {//extended ID used
            ...
            }//当pMsgObject->ulMsgIDMask == 0x1fffffff的时候不需要再考虑那个MSG_OBJ_USE_ID_FILTER
			//ulFlags&MSG_OBJ_USE_ID_FILTER == 0说明该消息对象收到的是一个远程的帧,需要过滤
			//总的说来当起符合以上两个条件中的任何一个条件时,需要使用ID FILTER
        }
        else
        {//standarded ID
            //
			...
			//if pMsgObject->ulMsgIDMask == 0x7ff,no ID FILTER
        }
    ...
    {//new data enabled
        ...
        if((pMsgObject->ulFlags & MSG_OBJ_REMOTE_FRAME) == 0)
        {
            ...
            // read the data[0]->data[7] from the register to the struct
            // of pMsgObject
