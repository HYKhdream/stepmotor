...
SysCtlClockSet(unsigned long ulConfig)
{
    ...
    {//1.内部晶振在寄存器设置中被禁用,在配置中被使用
    	//2.主晶振在寄存器设置中被禁用,在配置中被开启
    		//意味着有别的晶振需要被开启
        //
		...
		//在寄存器RCC中,其bit[1]&0 bit[0]&0 是将相应的晶振使能
		//根据ulconfig的值来是否需要使能别的的晶振
    ...
    {//use the pLL output clock as the source clock
        //
        ...
        for(ulDelay = 32768; ulDelay > 0; ulDelay--)
        {
            ...
            {//it means that the PLL is ready
                break;
            }
        }
