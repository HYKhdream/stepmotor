               ...
               unsigned long ulPinIO)
{
    ...
    HWREG(ulPort + GPIO_O_DIR) = ((ulPinIO & 1) ?
                                  ...
                                  (HWREG(ulPort + GPIO_O_DIR) & ~(ucPins)));
	// configure the goio just to be output or input

    HWREG(ulPort + GPIO_O_AFSEL) = ((ulPinIO & 2) ?
                                    (HWREG(ulPort + GPIO_O_AFSEL) | ucPins) :
                                    (HWREG(ulPort + GPIO_O_AFSEL) &
                                     ~(ucPins)));
	// configure the gpio to be controled by the hardware
}
...
void
GPIOSetUnlock(unsigned long ulPort)
{
    //
    // Check the arguments.
    //
    ...
    // Set the pin direction and mode.
    //
    HWREG(ulPort + GPIO_O_LOCK) = 0x4C4F434B;
    HWREG(ulPort + GPIO_O_CR) = 0xff;
}

void
GPIOSetLock(unsigned long ulPort)
{
    //
    // Check the arguments.
    //
