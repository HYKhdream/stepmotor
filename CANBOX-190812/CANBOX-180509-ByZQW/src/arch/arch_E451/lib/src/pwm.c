                ...
                unsigned long ulPeriod)
{
    ...
    // we need to judge the countmode before we load the count value to the register
    if(HWREG(ulGen + PWM_O_X_CTL) & PWM_X_CTL_MODE)
    {//continue count from 0 to load_value then from load_value to 0
        //
    ...
    {//continue count from the load_value to 0
        //
